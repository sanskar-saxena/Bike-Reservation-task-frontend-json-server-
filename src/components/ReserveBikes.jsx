...
const ReserveBikes = () => {
  ...
  const bookHandler = () => {
    if(startDateX.getTime()< bike.startDate || startDateX.getTime()> bike.endDate || endDateX.getTime()<bike.startDate || endDateX.getTime()>bike.endDate){
      ...
      dispatch(
        createReservation({
          bikeId: +params.bikeId,
          userId: userInfo.user.id,
          start_date: formatDate(startDateX),
          end_date: formatDate(endDateX),
          status: "BOOKED",
        })
      );
      dispatch(
        ...
        <>
          <Row>
            ...
            <Col md={3}>
              ...
              {success && (
                <Message variant="success">{"Reservation Created Successfully"}</Message>
              )}

              <ListGroup variant="flush">
                
                <ListGroup.Item>
                  <strong>Available from {dateConverter(bike.startDate)}-{dateConverter(bike.endDate)}</strong>
                </ListGroup.Item>                
                <ListGroup.Item>
                  <h3>{bike.model}</h3>
                </ListGroup.Item>
