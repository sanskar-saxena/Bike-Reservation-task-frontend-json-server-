...
export const createReview = (review) => async (dispatch, getState) => {
  try {
    ...
    const { data: reservations } = await axios.get(
      `https://bike-bungy.herokuapp.com/600/users/${userInfo.user.id}/reservations`,
      headerInfo
    );
    ...
    const { data: userReviews } = await axios.get(
      `https://bike-bungy.herokuapp.com/users/${userInfo.user.id}/reviews`,
      review,
      headerInfo
    );
    ...
    const { data } = await axios.post(
      `https://bike-bungy.herokuapp.com/bike/${review.bikeId}/reviews`,
      review,
      headerInfo
    );
...
export const reviewList = (bikeId) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`https://bike-bungy.herokuapp.com/bikes/${bikeId}/reviews`, headerInfo);
