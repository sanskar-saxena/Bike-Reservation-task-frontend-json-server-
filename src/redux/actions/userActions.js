...
export const USER_RESERVATION_LIST_FAIL_RESET = "USER_RESERVATION_LIST_FAIL_RESET";
...
export const logIn = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      "https://bike-bungy.herokuapp.com/login",
      { email, password },
      headerInfo
    );
      console.log(data);
...
export const signUp =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        "https://bike-bungy.herokuapp.com/signup",
        { email, password, role },
        headerInfo
      );
      console.log(data);
  ...
  export const userReservations = (id) => async(dispatch, getState) =>{
      try {
          ...
          const {data} = await axios.get(`https://bike-bungy.herokuapp.com/600/users/${+id}/reservations`, headerInfo);
  ...
  export const allUsers = (page, limit = 9) => async(dispatch, getState)=>{
    try {
        dispatch({
          type: USERS_LIST_REQ,
        });
        ...
        const res = await axios.get(`https://bike-bungy.herokuapp.com/users?_page=${page}&_limit=${limit}`, headerInfo);
        console.log(res);
        dispatch({
            type : USERS_LIST_SUCCESS,
            payload : {data : res.data, totalCount : res.headers["x-total-count"]}
        })
    }
    catch(error){
      console.log(error)
  ...
  export const userDetails = (id) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.get(`https://bike-bungy.herokuapp.com/users/${id}`, headerInfo);
  ...
  export const updateUser = (user) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.patch(`https://bike-bungy.herokuapp.com/users/${user.id}`, user, headerInfo);
  ...
  export const deleteUser = (id) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.delete(`https://bike-bungy.herokuapp.com/users/${id}`, headerInfo);
  ...
  export const createUser = (user) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.post("https://bike-bungy.herokuapp.com/signup", user, headerInfo);
